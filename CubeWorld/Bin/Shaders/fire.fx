...
GeometryShader gsStreamOut = ConstructGSWithSO( CompileShader( gs_4_0, StreamOutGS() ), 
	"POSITION.xyz; VELOCITY.xyz; SIZE.xy; AGE.x; TYPE.x; COLOR.xyz" );
...
VS_OUT DrawVS(Particle vIn)
{
	...
	float opacity = 0.9f - smoothstep(0.0f, 1.0f, t/1.0f);
	vOut.color = float4(vIn.color.x, vIn.color.y, vIn.color.z, opacity);
...
{	
	...
	if( gIn[0].type != PT_EMITTER )
	{
		...
		// Compute 4 triangle strip mVertices (quad) in local space.
		...
		// Transform quad mVertices to world space and output 
