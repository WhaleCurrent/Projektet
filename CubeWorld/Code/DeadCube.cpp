...
DeadCube::DeadCube(string filename, ID3D10Device* device, ID3D10ShaderResourceView* diff, ID3D10ShaderResourceView* spec, D3DXVECTOR3 position, D3DXVECTOR3 rotation, D3DXVECTOR3 scale)
{
...
void DeadCube::Update()
{
	if(HUDdeath && !HUDNoTime)
	{
		...
		D3DXMatrixTranslation(&T, sinf(mPosition.x), mPosition.y, cosf(mPosition.z));
		W = gCubeMatrix;
		W = W * T;
	}
	else
