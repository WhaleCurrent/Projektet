...
HRESULT CWaveFile::Open( char* strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
        {
            ...
            m_pResourceBuffer = myNew CHAR[ dwSize ];
            if( m_pResourceBuffer == NULL )
                return DXTRACE_ERR( "myNew", E_OUTOFMEMORY );
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    {
        m_pwfx = (WAVEFORMATEX*)myNew CHAR[ sizeof(WAVEFORMATEX) ];
        if( NULL == m_pwfx )
    ...
    else
    {
        ...
        m_pwfx = (WAVEFORMATEX*)myNew CHAR[ sizeof(WAVEFORMATEX) + cbExtraBytes ];
        if( NULL == m_pwfx )
            return DXTRACE_ERR( "myNew", E_FAIL );
...
// Desc: Support function for reading from a multimedia I/O stream
//       pwfxDest is the WAVEFORMATEX for this myNew wave file.
//       m_hmmio must be valid before calling.  This function uses it to
