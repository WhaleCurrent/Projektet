...
void Particle2::init(ID3D10Device* device, ID3D10Effect* effects, UINT maxParticles, ID3D10ShaderResourceView* diff)
{
	isFire = false;
	md3dDevice = device;
...
void Particle2::update(float deltaTime, float gameTime)
{
	mGameTime = gameTime;
	mTimeStep = deltaTime;
...
void Particle2::draw()
{
	...
	mEmitPosVar->SetFloatVector((float*)&gParticleEmitPosMove);
	mRandomTexVar->SetResource(mRandomTex);
	mTextureVar->SetResource(mTexture);
	mUpdateVar->SetBool(updateParticleMove);
	mParticleColorVar->SetFloatVector((float*)&gParticleColorMove);
	mSizeVar->SetFloatVector((float*)&gParticleSizeMove);

	md3dDevice->IASetInputLayout(InputLayout::mParticleMove);
